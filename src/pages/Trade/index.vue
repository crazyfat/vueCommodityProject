<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="(address, index) of userAddressList" :key="address.id">
      ...
      <div class="detail">
        <h5>商品清单</h5>
        <ul class="list clearFix" v-for="(list, index) of userOrder.detailArrayList" :key="list.skuId">
          <li>
            <img style="width: 82px; height:82px" :src="list.imgUrl"/>
          </li>
          <li>
            <p>{{list.skuName}}</p>
            <h4>7天无理由退货</h4>
...
<script>
  ...
  export default {
    ...
    methods:{
      changeDefault(index){
        this.userAddressList.forEach((li, id)=>{
          if(id==index){
            li.isDefault = 1
          }else{
            li.isDefault = 0
      ...
      submitOrder(){
        ...
        this.$API.reqSubmitOrder(tradeNo, data).then(
            value =>{
              if(value.code==200){
                console.log('提交订单成功')
                this.orderId = value.data
                this.$router.push(`/pay/${value.data}`)
              }else{
                console.log(value)
                alert('订单提交失败')
              }
    ...
    computed:{
      ...mapState('tradeStore', ['userOrder', 'userAddressList']),
      userDefaultAddress(){
        return this.userAddressList.find(item=>item.isDefault==1) || {}
      }
    },
    mounted() {
      this.$store.dispatch('tradeStore/getUserAddress')
      this.$store.dispatch('tradeStore/getUserOrder')
    }
  }
</script>
